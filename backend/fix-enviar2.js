...
// Substituir o endpoint antigo
content = content.replace(
    /app\.post\('\/api\/whatsapp\/enviar', auth, async \(req, res\) => \{[\s\S]*?res\.status\(500\)\.json\(\{ error: 'Erro ao enviar' \}\);\s*\}\s*\}\);/,
    novoEndpoint
);

fs.writeFileSync('server.js', content);
console.log('Endpoint /enviar atualizado com suporte Z-API e WasenderAPI!');
